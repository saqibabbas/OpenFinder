...
public class ImageUtils {
	...
	public static double[] createArrFromIm(BufferedImage im) {
		...
		im = ImageUtils.toScaledGray(im, 1.0);
	...
	public static void checkImSizes(ArrayList<String> fNms, BufferedImage[] ims) {
		...
		for (int i = 1; i < ims.length; i++) {
			if ((ims[i].getHeight() != imHeight) || (ims[i].getWidth() != imWidth)) {
				System.out.println("All images should have be the same size; " + fNms.get(i) + " is a different size");
				System.exit(1);
			}
		}
	} // end of checkImSizes()
