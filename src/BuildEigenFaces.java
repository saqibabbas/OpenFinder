...
public class BuildEigenFaces {
	...
	// new build method
	public void build(int numEFs, String path) {
		ArrayList<String> fnms = FileUtils.getTrainingFnms(path);
		if (fnms != null) {
			int numIms = fnms.size();
			numEFs = numIms - 1;
			if ((numEFs < 1) || (numEFs >= numIms)) {
				System.out.println(
						"Number of eigenfaces must be in range (1-" + (numIms - 1) + ")" + "; using " + (numIms - 1));
				numEFs = numIms - 1;
			} else
				System.out.println("Number of eigenfaces: " + numEFs);

			FaceBundle bundle = makeBundle(fnms);
			FileUtils.writeCache(bundle, path);
			// reconstructIms(numEFs, bundle); // optional: rebuild the original
			// images
			// from the bundle
		}
	...
	private FaceBundle makeBundle(ArrayList<String> fnms) {
		BufferedImage[] ims = FileUtils.loadTrainingIms(fnms);

		Matrix2D imsMat = convertToNormMat(ims); // each row is a normalized
													// image
		double[] avgImage = imsMat.getAverageOfEachColumn();
		imsMat.subtractMean(); // subtract mean face from each image (row)
		// each row now contains only distinguishing features from a training
		// image
		...
		// for saving eigen faces
		// FileUtils.saveEFIms(egFaces, ims[0].getWidth());

		// System.out.println("Saving done\n");

		return new FaceBundle(fnms, imsMat.toArray(), avgImage, egFaces.toArray(), egVals, ims[0].getWidth(),
				ims[0].getHeight());
	} // end of makeBundle()

	private Matrix2D convertToNormMat(BufferedImage[] ims)
	/*
	 * convert array of images into a matrix; each row is an image and the
	 * number of columns is the number of pixels in the image. The array is
	 * normalized.
	 */
	{
		...
		for (int i = 0; i < numRows; i++)
			ims[i].getData().getPixels(0, 0, imWidth, imHeight, data[i]); // one
																			// image
																			// per
																			// row
	...
	private Matrix2D getNormEgFaces(Matrix2D imsMat, Matrix2D egVecs)
	/*
	 * calculate normalized Eigenfaces for the training images by multiplying
	 * the eigenvectors to the training images matrix
	 */
	...
	private void sortEigenInfo(double[] egVals, double[][] egVecs)
	/*
	 * sort the Eigenvalues and Eigenvectors arrays into descending order by
	 * eigenvalue. Add them to a table so the sorting of the values adjusts the
	...
	private Double[] getEgValsAsDoubles(double[] egVals)
	...
	private double[] getColumn(double[][] vecs, int col)
	/*
	 * the Eigenvectors array is in column order (one vector per column); return
	 * the vector in column col
	 */
	{
	...
	private ArrayList<Double> sortKeysDescending(Hashtable<Double, double[]> table)
	...
	private void updateEgVecs(double[][] egVecs, Hashtable<Double, double[]> table, Double[] egDvals,
			ArrayList<Double> sortedKeyList)
	/*
	 * get vectors from the table in descending order of sorted key, and update
	 * the original vectors array
	 */
	{
	...
	// private void reconstructIms(int numEFs, FaceBundle bundle) {
	...
	// double[][] normImgs = getNormImages(weights, egFacesSubMat,
	// egValsSubMat);
	...
	// private double[][] getNormImages(double[][] weights, Matrix2D
	// egFacesSubMat, Matrix2D egValsSubMat)
	// /*
	// * calculate weights x eigenfaces, which generates mean-normalized
	// traimning
	// * images; there is one image per row in the returned array
	// */
	// {
	...
	//
	// private double[][] addAvgImage(double[][] normImgs, double[] avgImage)
	// // add the average image to each normalized image (each row) and store in
	// a
	// // new array;
